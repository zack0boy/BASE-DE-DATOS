...
@Component({
  selector: 'app-estudiantes-contenido',
  standalone: true,
  imports: [
    CommonModule,
    RouterModule,
      ...
      map(data => {
        ...
        if (Array.isArray(data.usuarios) && data.usuarios.length > 0) {
          ...
          if (Array.isArray(data.docentes) && data.docentes.length > 0) {
            ...
            // Si no hay asignaciones específicas, mostrar todos los docentes
            console.log('Sin asignaciones específicas, mostrando todos los docentes');
